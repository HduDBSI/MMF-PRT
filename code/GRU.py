...
def load_and_preprocess_numeric_only(
    ...
    X_train = np.expand_dims(X_train, axis=-1)   # [N, 18, 1]
    X_val = np.expand_dims(X_val, axis=-1)       # [N, 18, 1]
...
# ---------------- GRU 回归模型 ----------------
class GRUNumericRegressor(nn.Module):
    ...
    def forward(self, x):
        ...
        _, h_n = self.gru(x)
        h_last = h_n[-1]                  # [B, H]
        pred = self.regressor(h_last)     # [B, 1]
...
if __name__ == '__main__':
    ...
    PROJECT_ID = 'org.apache:commons-io'
    SEED = 42
